import type { LessonContent } from "../types";

export const lessonData: LessonContent = {
  title: "Nhà nước của nhân dân, do nhân dân, vì nhân dân",
  summary: [
    "Tư tưởng Hồ Chí Minh về Nhà nước của nhân dân, do nhân dân, vì nhân dân là một nội dung cốt lõi trong tư tưởng chính trị của Người.",
    "Quan điểm này thể hiện rõ bản chất, vai trò, chức năng của Nhà nước Việt Nam Dân chủ Cộng hòa và tinh thần dân chủ sâu sắc, tình yêu thương vô bờ đối với nhân dân.",
    "Tư tưởng được cụ thể hóa thành các nguyên tắc, hình thức thực thi quyền lực và phương thức xây dựng nhà nước trong sạch, vững mạnh.",
  ],
  videos: [
    {
      id: "hcm-state-of-nation",
      title: "Việt Nam thời đại Hồ Chí Minh – Nhà nước của dân, do dân, vì dân",
      description:
        "Video tài liệu phân tích tư tưởng Hồ Chí Minh về nhà nước dân chủ xuyên suốt từ Cách mạng Tháng Tám đến Hiến pháp 1946.",
      embedUrl: "https://www.youtube.com/embed/Ya3k9WTQMKk",
      thumbnailUrl: "https://img.youtube.com/vi/Ya3k9WTQMKk/maxresdefault.jpg",
      duration: "12:20",
      category: "documentary",
    },
    {
      id: "hcm-democracy-thought",
      title: "Tư tưởng Hồ Chí Minh về dân chủ và quyền làm chủ của nhân dân",
      description:
        "Bài giảng/clip phân tích sâu về quan điểm của Chủ tịch Hồ Chí Minh về dân chủ, quyền lực thuộc về nhân dân và trách nhiệm của nhà nước.",
      embedUrl: "https://www.youtube.com/embed/1E1zPJPKRyI",
      thumbnailUrl: "https://img.youtube.com/vi/1E1zPJPKRyI/maxresdefault.jpg",
      duration: "4:52",
      category: "lecture",
    },
    {
      id: "hcm-constitution-1946",
      title: "Hiến pháp năm 1946 – Mang ý chí chung của dân tộc",
      description:
        "Video trình bày quá trình soạn thảo và giá trị lịch sử của Hiến pháp 1946, nêu bật vai trò của tư tưởng Hồ Chí Minh trong việc thể chế hóa 'của dân, do dân, vì dân'.",
      embedUrl: "https://www.youtube.com/embed/CWAVgWlO2Ds",
      thumbnailUrl: "https://img.youtube.com/vi/CWAVgWlO2Ds/maxresdefault.jpg",
      duration: "4:05",
      category: "historical",
    },
  ],
  timeline: [
    {
      year: "1911",
      event: "Ra đi tìm đường cứu nước",
      description:
        "Ngày 5/6/1911, Nguyễn Tất Thành rời Bến Nhà Rồng, lên tàu Amiral Latouche-Tréville sang Pháp và nhiều nước khác để tìm con đường giải phóng dân tộc.",
      significance:
        "Mở đầu hành trình tiếp cận các tư tưởng tiến bộ về dân chủ, tự do, bình đẳng.",
    },
    {
      year: "1919",
      event: "Gửi Bản yêu sách của nhân dân An Nam",
      description:
        "Thay mặt Hội những người Việt Nam yêu nước tại Pháp, Nguyễn Ái Quốc gửi bản 'Yêu sách của nhân dân An Nam' tới Hội nghị Versailles.",
      significance:
        "Lần đầu tiên công khai nêu yêu cầu về quyền tự quyết và tự do cho dân tộc Việt Nam.",
    },
    {
      year: "1920",
      event: "Tham gia Đại hội Tours",
      description:
        "Nguyễn Ái Quốc tham gia Đại hội Tours (Đảng Xã hội Pháp) và bỏ phiếu tán thành gia nhập Quốc tế Cộng sản.",
      significance:
        "Tiếp cận chính thức với chủ nghĩa Mác-Lê-nin, xác định con đường cách mạng vô sản.",
    },
    {
      year: "1923",
      event: "Sang Liên Xô",
      description:
        "Nguyễn Ái Quốc sang Liên Xô học tập và nghiên cứu tại Trường Đại học Phương Đông.",
      significance:
        "Tiếp thu sâu sắc lý luận về nhà nước công nông và dân chủ nhân dân.",
    },
    {
      year: "1930",
      event: "Thành lập Đảng Cộng sản Việt Nam",
      description:
        "Ngày 3/2/1930, tại Hồng Kông, Nguyễn Ái Quốc chủ trì hợp nhất ba tổ chức cộng sản thành Đảng Cộng sản Việt Nam.",
      significance:
        "Đặt nền tảng tổ chức lãnh đạo cách mạng và định hướng tư tưởng dân chủ cho nhà nước tương lai.",
    },
    {
      year: "1941",
      event: "Về nước lãnh đạo cách mạng",
      description:
        "Nguyễn Ái Quốc trở về Pác Bó (Cao Bằng), trực tiếp lãnh đạo phong trào giải phóng dân tộc.",
      significance:
        "Gắn kết tư tưởng giải phóng dân tộc với xây dựng nhà nước của dân.",
    },
    {
      year: "1945",
      event: "Tuyên ngôn độc lập",
      description:
        "Ngày 2/9/1945, Hồ Chí Minh đọc Tuyên ngôn độc lập tại Quảng trường Ba Đình, khai sinh nước Việt Nam Dân chủ Cộng hòa.",
      significance:
        "Khẳng định quyền dân tộc tự quyết, quyền dân chủ và bình đẳng của nhân dân Việt Nam.",
    },
    {
      year: "1946",
      event: "Hiến pháp 1946",
      description:
        "Quốc hội thông qua Hiến pháp đầu tiên của nước Việt Nam độc lập.",
      significance: "Thể chế hóa nguyên tắc 'của dân, do dân, vì dân'.",
    },
    {
      year: "1960",
      event: "Đại hội Đảng toàn quốc lần thứ III",
      description:
        "Hồ Chí Minh chỉ đạo đường lối xây dựng CNXH ở miền Bắc và đấu tranh thống nhất nước nhà.",
      significance:
        "Tiếp tục phát triển tư tưởng dân chủ gắn với xây dựng xã hội mới.",
    },
    {
      year: "1969",
      event: "Di chúc của Chủ tịch Hồ Chí Minh",
      description:
        "Hồ Chí Minh để lại Di chúc, thể hiện tâm nguyện và định hướng cho tương lai đất nước.",
      significance:
        "Khẳng định lại niềm tin vào nhân dân và nguyên tắc dân chủ.",
    },
  ],
  keyQuotes: [
    {
      text: "Dân là gốc của nước, gốc vững thì nước mới yên.",
      context:
        "Phát biểu tại Đại hội đại biểu toàn quốc lần thứ II của Đảng (1951)",
      significance:
        "Khẳng định vai trò cơ bản của nhân dân trong việc xây dựng và bảo vệ đất nước",
      link: "https://www.truongchinhtrikontum.org.vn/quan-diem-nuoc-lay-dan-lam-goc-trong-tu-tuong-ho-chi-minh-va-su-van-dung-cua-dang-ta",
    },
    {
      text: "Nước ta là nước dân chủ, địa vị cao nhất là dân, vì dân là chủ.",
      context: "Nói chuyện với cán bộ Bộ Nội vụ (1948)",
      significance: "Làm rõ bản chất dân chủ của nhà nước Việt Nam",
      link: "https://thanhnien.vn/dan-la-chu-va-dan-lam-chu-trong-tu-tuong-ho-chi-minh-1851459851.htm",
    },
    {
      text: "Các công việc của Chính phủ làm phải nhằm vào một mục đích duy nhất là mưu tự do hạnh phúc cho mọi người.",
      context: "Thư gửi đồng bào Nam bộ (1946)",
      significance:
        "Xác định mục tiêu cao nhất của nhà nước là phục vụ nhân dân",
      link: "https://tcnn.vn/news/detail/43282/Triet-ly-xay-dung-Chinh-phu-kien-tao-trong-tu-tuong-Ho-Chi-Minh.html",
    },
    {
      text: "Bao nhiêu lợi ích đều vì dân. Bao nhiêu quyền hạn đều của dân.",
      context: "Bài 'Dân vận' đăng trên báo Sự Thật (15/10/1949)",
      significance:
        "Khẳng định quyền làm chủ và quyền lợi tối cao của nhân dân",
      link: "https://www.tapchicongsan.org.vn/web/guest/nghien-cu/-/2018/821308/tac-pham-%E2%80%9Cdan-van%E2%80%9D-cua-chu-tich-ho-chi-minh-va-y-nghia-doi-voi-cong-tac-dan-van-cua-dang-hien-nay.aspx",
    },
    {
      text: "Việc gì lợi cho dân, ta phải hết sức làm. Việc gì hại đến dân, ta phải hết sức tránh.",
      context: "Bài 'Dân vận' đăng trên báo Sự Thật (15/10/1949)",
      significance:
        "Đặt lợi ích của nhân dân làm tiêu chí tối cao trong hoạt động của nhà nước",
      link: "https://nhandan.vn/viec-gi-loi-cho-dan-ta-phai-het-suc-lam-post615409.html",
    },
    {
      text: "Cán bộ là đầy tớ của dân, dân là chủ.",
      context: "Phát biểu trong buổi tiếp xúc cử tri Hà Nội (1956)",
      significance: "Làm rõ mối quan hệ giữa nhà nước và nhân dân",
      link: "https://nghean.dcs.vn/vi-vn/tin/quan-diem-cua-chu-tich-ho-chi-minh-ve-dan-chu-va-thuc-hanh-dan-chu--tang-cuong-phap-che--dam-bao-ky-cuong-xa-hoi/73410-894751-415577",
    },
    {
      text: "Tôi chỉ có một ham muốn, ham muốn tột bậc, là làm sao cho nước ta được hoàn toàn độc lập, dân ta được hoàn toàn tự do, ai cũng có cơm ăn, áo mặc, ai cũng được học hành.",
      context: "Trả lời phỏng vấn của nhà báo Pháp (1946)",
      significance:
        "Thể hiện mục tiêu toàn diện của cách mạng về độc lập, tự do và hạnh phúc cho nhân dân",
      link: "https://nhandan.vn/ham-muon-tot-bac-cua-bac-ho-post421662.html",
    },
    {
      text: "Dễ trăm lần không dân cũng chịu, khó vạn lần dân liệu cũng xong.",
      context: "Lời dạy trong công tác dân vận",
      significance:
        "Khẳng định sức mạnh và vai trò quyết định của nhân dân trong mọi công việc",
      link: "https://baotuyenquang.com.vn/Ke-chuyen-ve-bac/201801/de-tram-lan-khong-dan-cung-chiu-kho-van-lan-dan-lieu-cung-xong-718696/",
    },
  ],
  interactiveElements: [
    {
      id: "reflection-1",
      type: "reflection",
      title: "Suy ngẫm về dân chủ",
      content:
        "Theo bạn, làm thế nào để thực hiện tốt nguyên tắc 'của dân, do dân, vì dân' trong xã hội hiện đại?",
    },
    {
      id: "poll-1",
      type: "poll",
      title: "Khía cạnh nào quan trọng nhất?",
      content:
        "Trong ba khía cạnh 'của dân, do dân, vì dân', bạn cho rằng điều nào quan trọng nhất?",
      options: [
        "Của dân - Quyền lực thuộc về nhân dân",
        "Do dân - Nhân dân trực tiếp tham gia",
        "Vì dân - Phục vụ lợi ích nhân dân",
      ],
    },
    {
      id: "discussion-1",
      type: "discussion",
      title: "Thảo luận nhóm",
      content:
        "Hãy thảo luận về cách áp dụng tư tưởng Hồ Chí Minh về nhà nước dân chủ vào thực tiễn Việt Nam hiện nay.",
    },
  ],
  images: [
    "https://cdn.luatminhkhue.vn/lmk/articles/71/357009/dan-chu-la-gi---khai-niem-ve-dan-chu-357009.jpg",
    "https://cly.1cdn.vn/2023/11/08/cover-loat-bai-xay-dung-nha-nuoc-pq-b1.jpg",
    "https://bcp.cdnchinhphu.vn/334894974524682240/2022/10/3/img0425-1664767833957315204122.jpg",
    "https://kiemsat.1cdn.vn/thumbs/600x315/2023/03/10/24hhoc.edu_.vn-tu-tuong-ho-chi-minh-ve-nha-nuoc-phap-quyen.png",
    "http://quocphongthudo.vn/upload/2001606/fck/files/image_71f35.png",
    "https://tse3.mm.bing.net/th/id/OIP.p-quJftP-6_02V3lnzOv5QHaEo?r=0&rs=1&pid=ImgDetMain&o=7&rm=3",
    "https://quocphongthudo.vn/upload/2001606/20200730/a8a51eb04dc4056f47dc4fe82f8e8e7c87eedb40d1.jpg",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcShu4Zi4ammbTfH-kFLCNrIdcTZtKYGmNGEbi_u7EHMmf5AeAwzP1HLy75c6HVEA9hIYsI&usqp=CAU",
    "https://hoanghamobile.com/tin-tuc/wp-content/uploads/2023/07/ngay-thanh-lap-Dang-Cong-san-Viet-Nam-4.jpg",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQI3jfuU2AY-Ro3G81k5d80OxM7a6wJKSCjig&s",
  ],
  cards: [
    {
      color: "red",
      title: "Nhà nước dân chủ",
      items: [
        {
          title: "Bản chất giai cấp của Nhà nước",
          content: [
            "Nhà nước Việt Nam Dân chủ Cộng hòa mang bản chất giai cấp công nhân, do Đảng Cộng sản Việt Nam lãnh đạo.",
            "Nòng cốt của nhân dân là liên minh công – nông – trí, dưới sự lãnh đạo của giai cấp công nhân và Đảng.",
            "Đảng cầm quyền thông qua:",
            "1) Đường lối, chủ trương được thể chế hóa thành pháp luật, chính sách.",
            "2) Hoạt động của tổ chức và đảng viên trong bộ máy nhà nước.",
            "3) Công tác kiểm tra, giám sát.",
            "- Nhà nước đồng thời có tính nhân dân và tính dân tộc sâu sắc.",
            "+ Là thành quả đấu tranh của toàn dân tộc.",
            "+ Không thuộc riêng giai cấp hay tầng lớp nào, mà là của nhân dân.",
          ],
        },
        {
          title: "Nhà nước của nhân dân",
          content: [
            "Nhân dân là chủ, mọi quyền lực thuộc về nhân dân.",
            "Quyền lực nhà nước là sự ủy quyền của nhân dân, cán bộ là “đầy tớ” chứ không phải “quan cách mạng”.",
            "Nhân dân thực hiện quyền làm chủ bằng:",
            "Dân chủ trực tiếp (tự quyết định các vấn đề quan trọng).",
            "Dân chủ gián tiếp (thông qua đại diện được bầu).",
            "Quyền của nhân dân:",
            "+ Quyết định tổ chức và hoạt động của bộ máy nhà nước",
            "+ Giám sát, bãi miễn cán bộ không xứng đáng",
            "+ Thực thi quyền lực qua pháp luật dân chủ.",
          ],
        },
        {
          title: "Nhà nước do nhân dân",
          content: [
            "Do nhân dân lập ra thông qua bầu cử dân chủ, bình đẳng, trực tiếp và bỏ phiếu kín.",
            "Nhân dân vừa có quyền làm chủ, vừa có nghĩa vụ công dân.",
            "Nhân dân tham gia quản lý, xây dựng và bảo vệ Nhà nước; muốn làm chủ tốt phải có năng lực làm chủ.",
          ],
        },
        {
          title: "Nhà nước vì nhân dân",
          content: [
            "Không có đặc quyền, đặc lợi; cán bộ là “công bộc của dân”, phải cần, kiệm, liêm, chính.",
            "Mục tiêu cao nhất: phục vụ lợi ích và hạnh phúc của nhân dân, “việc gì lợi cho dân thì làm, việc gì hại cho dân thì tránh”",
            "Nhà nước vì dân phải được lòng dân: yêu dân, đặt lợi ích của dân lên trên hết, chí công vô tư.",
          ],
        },
      ],
    },
    {
      color: "yellow",
      title: "Nhà nước pháp quyền",
      items: [
        {
          title: "Nhà nước hợp hiến, hợp pháp",
          content: [
            "- Hồ Chí Minh luôn chú trọng vấn đề xây dựng nền tảng pháp lý cho Nhà nước Việt Nam mới.",
            "+ Tổ chức và hoạt động dựa trên Hiến pháp và pháp luật",
            "+ Quyền lực nhà nước được phân công, phối hợp và kiểm soát theo luật định",
          ],
        },
        {
          title: "Nhà nước thượng tôn pháp luật",
          content: [
            "- Mọi cá nhân, tổ chức, kể cả Nhà nước và cán bộ đều phải tuân theo pháp luật.",
            '-> Hồ Chí Minh tuyên bố: "Pháp luật Việt Nam tuy khoan hồng với những người biết cải tà quy chính, nhưng sẽ thẳng tay trừng trị những tên Việt gian đầu sỏ đã bán nước buôn dân"',
            "- Điều này đòi hỏi pháp luật phải công bằng, minh bạch, nghiêm minh.",
          ],
        },
        {
          title: "Pháp quyền nhân nghĩa",
          content: [
            "Nhà nước phải tôn trọng và bảo đảm quyền con người, chăm lo lợi ích cho mọi người.",
            "Pháp luật vừa nghiêm khắc vừa nhân đạo, bảo vệ quyền lợi chính đáng của nhân dân.",
            "Kết hợp kỷ cương pháp luật với tình thương, đạo lý; pháp luật vì con người.",
          ],
        },
      ],
    },
    {
      color: "green",
      title: " Nhà nước trong sạch, vững mạnh",
      items: [
        {
          title: "Kiểm soát quyền lực nhà nước",
          content: [
            "- Quyền lực phải được phân công, phối hợp, kiểm soát để tránh lạm quyền",
            "- Cơ chế giám sát của nhân dân và các tổ chức chính trị – xã hội",
          ],
        },
        {
          title: "Phòng chống tiêu cực trong Nhà nước",
          content: [
            "Hồ Chí Minh cảnh báo các tiêu cực: đặc quyền, đặc lợi; tham nhũng, quan liêu, lãng phí – coi đây là “giặc nội xâm”, nguy hại như Việt gian, mật thám.",
            "Biện pháp phòng chống:",
            "Phát huy dân chủ, quyền làm chủ của nhân dân.",
            "Pháp luật, kỷ luật nghiêm minh; kiểm tra thường xuyên; xây dựng đội ngũ cán bộ cần, kiệm, liêm, chính.",
            "Trừng phạt nghiêm minh, đúng người đúng tội.",
            "Giáo dục đạo đức, nâng cao năng lực và nêu gương của cán bộ.",
            "Huy động sức mạnh yêu nước để chống tiêu cực trong xã hội và bộ máy nhà nước.",
          ],
        },
      ],
    },
  ],
};

export default lessonData;
